...
public class EAN128LogicImpl { // extends Code128LogicImpl{
	...
	public EAN128LogicImpl(final ChecksumMode mode, final String template, final char fnc1) {
		setChecksumMode(mode);
		setTemplate(template);
		this.groupSeparator = fnc1;
	}

	public EAN128LogicImpl(final ChecksumMode mode, final String template) {
		setChecksumMode(mode);
		setTemplate(template);
	}
	 ...
	 * @see #getEncoder()
	 */
	public int[] getEncodedMessage(final String msg) {
		setMessage(msg);
		return encodedMsg;
	}
	 ...
	 *
	 * @param logic
	 *            the logic handler to receive the generated events
	 * @param msg
	 *            the message to encode
	 */
	public void generateBarcodeLogic(final ClassicBarcodeLogicHandler logic, final String msg) {
		setMessage(msg);

		Code128LogicImpl c128 = new Code128LogicImpl();
		logic.startBarcode(msg, getHumanReadableMsg());
		for (int element : encodedMsg) {
			c128.encodeChar(logic, element);
		}
	...
	public void addAIs(final String msg) {
		int offset = 0;
		int i = 0;
		EAN128AI ai = null;
	...
	private int findGroupSeparator(final String msg, final int start) {
	...
	public int addAI(final String msg, final int offset, EAN128AI ai) {
		if (msg == null) {
		...
		if (newOffset < offset + lenID + lenMin) {
			if (checksumADD && ai.canDoChecksumADD && newOffset == offset + lenID + lenMin - 1) {
				...
				throw getException(
						"Field \"" + msg.substring(offset + lenID, newOffset) + "\" too short! Length should be " + lenMin + " at least!");
			}
		...
		if (newOffset > offset + lenID + lenMax) {
			if (ai.fixed || lenMin == lenMax) {
				...
				throw getException("Variable length field \"" + msg.substring(offset + lenID, newOffset) + "\" too long! Length should be "
						+ lenMax + " at the most!");
			}
		}
	...
	private boolean isGroupSeparator(final char ch) {
		return ch == groupSeparator || ch == Code128LogicImpl.FNC_1;
	}

	private void checkType(final EAN128AI ai, final byte idx, final String msg, final int start, final int end, final int cdStart) {
	...
	private char getIDChar(final String msg, final int offset) {
		char ret;
		try {
	...
	private IllegalArgumentException getException(final String text) {
	...
	public void setChecksumMode(final ChecksumMode mode) {
	 ...
	 * @param c
	 */
	public void setGroupSeparator(final char c) {
		groupSeparator = c;
	}
	 ...
	 * @param string
	 */
	public void setTemplate(final String string) {
	 ...
	 * @param c
	 */
	public void setCheckDigitMarker(final char c) {
		checkDigitMarker = c;
	}
	 ...
	 * @param b
	 */
	public void setOmitBrackets(final boolean b) {
		omitBrackets = b;
	}
