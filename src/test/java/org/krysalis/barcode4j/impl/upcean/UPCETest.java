...
public class UPCETest extends TestCase {
	...
	public UPCETest(final String name) {
		super(name);
	}
	...
	public void testMessageCompaction() throws Exception {
		for (String[] element : COMPACTION_TESTS) {
			assertEquals(
					element[1] + " must be compacted to "
							+ element[0],
							element[0],
							UPCELogicImpl.compactMessage(element[1]));
			String nocheck = element[1].substring(0, 11);
			assertEquals(
	...
	public void testMessageExpansion() throws Exception {
		for (String[] element : COMPACTION_TESTS) {
			assertEquals(
					element[0] + " must be expanded to "
							+ element[1],
							element[1],
							UPCELogicImpl.expandMessage(element[0]));
			String nocheck = element[0].substring(0, 7);
			assertEquals(
